...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
     ...
     */
	int max_array_size = keyboards_count * drives_count;
	int array[max_array_size];
	int k = 0, maxspent = 0;

	for(int i = 0; i < keyboards_count; i++)
	{
		for(int j = 0; j < drives_count; j++, k++)
		{
			array[k] = *(keyboards + i) + *(drives + j);
		}
	}
	for(int k = 0; k < max_array_size; k++)
	{
		if ((array[k] <= b) && (array[k] > maxspent))
		{
			maxspent = array[k];
		}
	}
	return maxspent ? maxspent : -1;
}
